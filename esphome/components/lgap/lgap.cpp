...
namespace esphome
{
  namespace lgap
  {
    ...
    void LGAP::loop()
    {
      ...
      if (this->debug_ == true) {
          if ((now - this->last_debug_time_) < this->debug_wait_time_) return;
      ...
      if (this->state_ == 0)
      {
        if (this->debug_ == true) ESP_LOGV(TAG, "State: 0");

        // 1. check if anything needs to be sent right now
        for (auto &device : this->devices_)
        {
          if (this->debug_ == true) ESP_LOGV(TAG, "Checking device %d for pending writes...", device->zone_number);

          if (device->zone_number > -1 && device->write_update_pending == true)
          {
            if (this->debug_ == true) ESP_LOGV(TAG, "write_update_pending==true for zone %d", device->zone_number);
            ...
            if (this->debug_ == true) ESP_LOGV(TAG, "write_update_pending==false for zone %d", device->zone_number);
          }
        }
        ...
        if ((now - this->last_zone_check_time_) > zone_check_wait_time_)
        {
          ...
          if (this->debug_ == true) ESP_LOGV(TAG, "Checking next zone");

          // cycle through zones
          this->last_zone_checked_index_ = (this->last_zone_checked_index_ + 1) > this->devices_.size() - 1 ? 0 : this->last_zone_checked_index_ + 1;
          if (this->debug_ == true) ESP_LOGV(TAG, "Checking zone index %d -> to zone number %d", this->last_zone_checked_index_, this->devices_[this->last_zone_checked_index_]->zone_number);
      ...
      if (this->debug_ == true) ESP_LOGV(TAG, "Available: %d, State: %d", this->available(), this->state_);

      // 3. read the response (only read when expecting a response)
      while (this->available() && this->state_ == 1)
      {
        if (this->debug_ == true) ESP_LOGV(TAG, "State: 1");
        if (this->debug_ == true) ESP_LOGV(TAG, "Trying to read...");
        ...
        {
          if (this->debug_ == true) ESP_LOGV(TAG, "LGAP received start of new response");
        ...
        if (this->state_ == 2)
        {
          if (this->debug_ == true) ESP_LOGV(TAG, "State: 2");
          ...
          {
            // todo: update last valid response received time
